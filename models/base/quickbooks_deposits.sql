with deposits as (
  ...
  select
    ...
    {% if target.type == 'bigquery' %}
      {{ dbt_utils.safe_cast('deposittoaccountref.value', dbt_utils.type_int()) }} as account_id,
      metadata.createtime as created_at,
      metadata.lastupdatedtime as updated_at,
    {% else %}
      {{ dbt_utils.safe_cast('deposittoaccountref__value', dbt_utils.type_int()) }} as account_id,
